...
void handleCommand(String msg) {
    ...
    // ── V4: Exotic Mode Toggle via network ────────────
      ...
      if (currentThemeId == 1) {
        playThemeTransition(1, 0); // Back to Legacy
      } else {
        playThemeTransition(currentThemeId, 1); // Jump to Exotic
      }
    }
    ...
    #define SAFESTR(dest, src, sz) do { const char* _s = (src).as<const char*>(); if (_s) { strncpy(dest, _s, sz-1); dest[sz-1] = '\0'; } } while(0)
      ...
      if (!deserializeJson(doc, json)) {
        // Primary ticker
        if (doc.containsKey("symbol")) SAFESTR(stockSymbol, doc["symbol"], 8);
        if (doc.containsKey("price"))  SAFESTR(stockPrice, doc["price"], 12);
        if (doc.containsKey("delta"))  SAFESTR(stockDelta, doc["delta"], 12);
        if (doc.containsKey("pct"))    SAFESTR(stockPctChg, doc["pct"], 10);
        ...
        if (doc.containsKey("wl")) {
          ...
          for (int i = 0; i < min((int)wl.size(), 3); i++) {
            SAFESTR(wlSymbol[i], wl[i]["s"], 8);
            SAFESTR(wlPrice[i], wl[i]["p"], 12);
            SAFESTR(wlDelta[i], wl[i]["d"], 10);
