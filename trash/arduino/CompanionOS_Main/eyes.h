...
// Emotion system defined in globals.h
...
#define EYE_H        38     // Baseline height (V3.1 original)
#define PUPIL_W      10     // V3.1 original
#define PUPIL_H      28     // V3.1 original
...
void drawAlmondEye(int cx, int cy, int w, int h, float pX, float pY,
  ...
  uint16_t spriteBg = (currentThemeId >= 2) ? activeTheme.bg : COLOR_BG;
  eyeSpr.fillSprite(spriteBg);
  int scx = sprW / 2;
  int scy = sprH / 2;
  ...
  if (exoticMode) {
    for (int sy = 0; sy < sprH; sy += 2) {
      for (int sx = 0; sx < sprW; sx += 3) {  // Skip pixels for perf
        ...
        if (pixel != spriteBg) {
          uint16_t dimmed = blendColor(pixel, spriteBg, 0.15);
...
void drawEyes() {
  // ═══════════════════════════════════════════════
  // V3.1 CORE ENGINE — Untouched almond geometry
  // All themes share these exact same eye shapes
  // ═══════════════════════════════════════════════

  // V5+: Draw theme-specific backgrounds (grid, noise, gradient, etc.)
  if (currentThemeId >= 2) {
    drawThemeBackground();
  }
...
void setEmotion(Emotion newEmotion) {
  if (currentEmotion != newEmotion) {
    ...
    if (currentState == STATE_EYES) {
      ...
      uint16_t clearBg = (currentThemeId >= 2) ? activeTheme.bg : COLOR_BG;
      tft.fillRect(0, eyeZoneY, SCREEN_W, eyeZoneH, clearBg);
      if (currentThemeId < 2) drawStarfield();
      drawEyes();
...
void updatePetPersonality() {
  ...
  if (isYawning) {
    ...
    if (elapsed < 800) {
      ...
      int eyeZoneY = 16;
      int eyeZoneH = SCREEN_H - 16;
      uint16_t yawnBg = (currentThemeId >= 2) ? activeTheme.bg : COLOR_BG;
      tft.fillRect(0, eyeZoneY, SCREEN_W, eyeZoneH, yawnBg);
      if (currentThemeId < 2) drawStarfield();
      drawEyes();
    }
  }
...
void updateEyes() {
  ...
  // Read LDR (on A0) with Auto-Calibration Smoothing
  static unsigned long lastLDR = 0;
  static unsigned long lastEmotionCheck = 0;
  static int ldrHistory[10] = {512,512,512,512,512,512,512,512,512,512};
  static int ldrIndex = 0;
  
  if (now - lastLDR > 500) {
    lastLDR = now;
    ldrHistory[ldrIndex] = analogRead(A0);
    ldrIndex = (ldrIndex + 1) % 10;
    long ldrSum = 0;
    for (int i = 0; i < 10; i++) ldrSum += ldrHistory[i];
    ldrValue = ldrSum / 10;
    
    if (now - lastEmotionCheck > 5000) {
  ...
  // ── UNIFIED V3.1 BLINK ENGINE (all themes) ──────────────
  ...
  if (isBlinking) {
    blinkPhase++;
    // V3.1: Per-eye column wipe — NEVER full-width
    ...
    // Redraw eyes beneath eyelid so shrinking eyelids expose the eye
    drawEyes();
    ...
    // V3.1 FIX: Wipe per-eye column ONLY — two small rects, never full-width
    uint16_t blinkBg = (currentThemeId >= 2) ? activeTheme.bg : COLOR_BG;
    tft.fillRect(LEFT_EYE_X - EYE_W/2 - 8, coverY, eyeColW, coverH, blinkBg);
    tft.fillRect(RIGHT_EYE_X - EYE_W/2 - 8, coverY, eyeColW, coverH, blinkBg);
  } else if (moved) {
    drawEyes();
  }
  skip_blink:;
