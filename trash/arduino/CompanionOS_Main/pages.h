...
void redrawSettingsPartial() {
  ...
  if (abs(smoothScrollY - settingsScrollY) > 0.5f) {
    smoothScrollY += (settingsScrollY - smoothScrollY) * 0.3f;
  } else {
    smoothScrollY = settingsScrollY;
  }
  
  tft.fillRect(0, 18, SCREEN_W, SCREEN_H - 35, COLOR_BG);
  
  int y = 25 - (int)smoothScrollY;
  ...
  for (int t = 0; t < THEME_COUNT; t++) {
    ...
    y += 32; // V6: Massive breathing room for theme UI
  }
  y += 12;  // Spacer after theme list
