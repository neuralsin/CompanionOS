...
class StockManager:
    ...
    def get_esp_payload(self) -> Dict:
        ...
        payload: Dict[str, Any] = {
            ...
            'price': primary['price'] if primary else '---',
        ...
        for sym in all_tickers:
            ...
            if q:
                ...
                self.wl_tree.insert('', 'end', values=(
                    q['symbol'], f"${q['price']}", q['delta'], q['pct'], status
                ), tags=(tag,))
    ...
    def _refresh_portfolio(self):
        ...
        for h in summary['holdings']:
            ...
            self.pf_tree.insert('', 'end', values=(
                h['symbol'], h['qty'], f"${h['avg']:.2f}",
                f"${h['current']:.2f}", f"${h['pnl']:+.2f}",
                f"{h['pnl_pct']:+.1f}%"
            ), tags=(tag,))
        ...
        self.portfolio_summary.config(
            text=f"Total: ${summary['total_current']:.2f}  |  "
                 f"P&L: ${pnl:+.2f} ({summary['total_pnl_pct']:+.1f}%)",
            style=color_style
        )
