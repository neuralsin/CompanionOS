...
void checkPhysicalSensors() {
  ...
  if (currentLeft == HIGH && lastLeftState == LOW && (now - lastLeftTrigger > CAP_DEBOUNCE_MS)) {
    lastLeftTrigger = now;
    lastInteractionTime = now;  // V4: Pet interaction
  ...
  if (currentState == STATE_EYES) {
    if ((currentLeft == HIGH || currentRight == HIGH) && 
        (now - lastLeftTrigger > 1500 || now - lastRightTrigger > 1500)) {
      if (!isPetting) {
        isPetting = true;
      ...
      // TAP detection - wider threshold for noisy touchscreens
        ...
        if (millis() - lastTouchTime > 300) {
          ...
          if (currentState == STATE_SPOTIFY) {
            ...
            int themeListEndY = themeListStartY + (THEME_COUNT * 20);
            
            if (lastTouchY >= max(18, themeListStartY) && lastTouchY < themeListEndY) {
              int tappedTheme = (lastTouchY - themeListStartY) / 20;
